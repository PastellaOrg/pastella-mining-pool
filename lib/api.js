...
function handleTopMiners (response) {
	async.waterfall([
		...
		function (workerKeys, callback) {
			...
			redisClient.multi(redisCommands).exec(function (error, redisData) {
					...
					for (let i in redisData) {
						...
						data = redisData[i];
						minersData.push({
							miner: address, // Full address for transparency
		...
		let reply = JSON.stringify({ miners: data });
...
function compareTopMiners (a, b) {
	// Sort by hashes (total work done) instead of hashrate
	let v1 = a.hashes ? parseInt(a.hashes) : 0;
	let v2 = b.hashes ? parseInt(b.hashes) : 0;
	if (v1 > v2) return -1;
	if (v1 < v2) return 1;
	return 0;
}
