...
let connectedMiners = {};
// Get merged mining tag reserved space size
...
function BlockTemplate (template) {
	...
	if (found_template_at !== -1) {
		...
		if (this.reserveOffset != template.reserved_offset) {
...
function processBlockTemplate (template) {
	...
	if (currentBlockTemplate[0]) {
		validBlockTemplates[0].push(currentBlockTemplate[0]);
	}

	while (validBlockTemplates[0].length > 3) {
		validBlockTemplates[0].shift();
	}

	currentBlockTemplate[0] = block_template;
...
function handleMinerMethod (method, params, ip, portData, sendReply, pushMessage) {
	...
	switch (method) {
		case 'login':
			...
			if (!utils.validateMinerAddress(login)) {
				let addressPrefix = login.substring(0, 10);
...
function recordShareData (miner, job, shareDiff, blockCandidate, hashHex, shareType, blockTemplate) {
	...
	if (workerName) {
		...
		redisCommands.push(['hset', `${coin}:unique_workers:${login}~${workerName}`, 'rewardType', rewardType]);
	...
	redisClient.multi(redisCommands)
		.exec(function (err, replies) {
			...
			if (blockCandidate) {
				...
				let totalScore = 0;
				let totalShares = 0;
				let minerScore = 0; // Individual miner's score for PPLNS calculation
				if (rewardType === 'solo') {
					totalScore = workerScores
					totalShares = workerShares
					minerScore = workerScores
				}
				if (rewardType === 'prop') {
					// Get the individual miner's score from the worker scores
					minerScore = workerScores && workerScores[login] ? parseFloat(workerScores[login]) : 0;
				...
				// Store participants list with their scores for candidate blocks
				if (workerScores && Object.keys(workerScores).length > 0) {
					...
					Object.keys(workerScores).forEach(worker => {
						...
						addressScores[address] += parseFloat(workerScores[worker]) || 0;
					});

					// Convert to "address:score,address:score" format
					let participants = Object.keys(addressScores)
						.map(addr => `${addr}:${addressScores[addr]}`)
						.join(',');

					redisClient.hset(coin + ':blocks:participants', job_height, participants);
				}
...
function processShare (miner, job, blockTemplate, params) {
	...
	if (hashDiff.ge(blockTemplate.difficulty)) {
		// Mark this height as being processed to prevent race conditions with duplicate blocks
		let processingKey = config.coin + ':blocks:processing:' + job.height;

		// Try to set a flag - if it already exists, another block is being processed for this height
		redisClient.set(processingKey, '1', 'PX', 5000, 'NX', function(err, result) {
			if (err) {
				log('error', logSystem, 'Error setting processing flag for height %d: %j', [job.height, err]);
				// Continue anyway
				submitBlock();
				return;
			}

			// If result is null, the key already exists (another block is being processed)
			if (result === null) {
				let blockFastHash = utils.cnUtil.get_block_id(shareBuffer, cnBlobType).toString('hex');
				log('info', logSystem, 'Block %s at height %d already submitted - counting share as valid work instead of block', [blockFastHash.substr(0, 12), job.height]);
				// Count as a valid share but not a block - miner did valid work, just on stale template
				recordShareData(miner, job, hashDiff.toString(), false, null, shareType, null);
				return;
			}

			// Successfully set the flag, submit this block
			submitBlock();
		});
		return true;
		...
		function submitBlock() {
			...
			if (error) {
				...
				// Pink success message for block submission
				log('info', logSystem, '\x1b[38;5;213mBlock has been found and submitted to daemon with success!\x1b[0m', []);
