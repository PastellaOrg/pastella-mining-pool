...
// let notifications = require('./notifications.js'); // REMOVED
 ...
 **/
function cleanupInvalidBlocks(callback) {
	async.waterfall([
		// Get current blockchain height
		function (innerCallback) {
			apiInterfaces.rpcDaemon('getblockcount', {}, function (error, result) {
				if (error) {
					log('error', logSystem, 'Error getting block count: %j', [error]);
					innerCallback(true);
					return;
				}
				innerCallback(null, result.count);
			});
		},
		// Get all block candidates
		function (currentHeight, innerCallback) {
			redisClient.zrange(config.coin + ':blocks:candidates', 0, -1, 'WITHSCORES', function (error, results) {
				if (error) {
					log('error', logSystem, 'Error getting pending blocks: %j', [error]);
					innerCallback(true);
					return;
				}
				if (results.length === 0) {
					log('info', logSystem, 'No block candidates to check');
					innerCallback(null, [], currentHeight);
					return;
				}
				...
				innerCallback(null, blocks, currentHeight);
			});
		},
		// Remove blocks with height >= current height (invalid)
		function (blocks, currentHeight, innerCallback) {
			let invalidBlocks = blocks.filter(function (block) {
				return block.height >= currentHeight;
			});

			if (invalidBlocks.length === 0) {
				log('info', logSystem, 'No invalid blocks found (all blocks below current height %d)', [currentHeight]);
				innerCallback(null, 0);
				return;
			}

			log('warn', logSystem, 'Found %d invalid blocks (height >= current height %d), removing...', [invalidBlocks.length, currentHeight]);
			invalidBlocks.forEach(function (block) {
				log('info', logSystem, 'Removing invalid block: hash=%s, height=%d', [block.hash, block.height]);
			});

			let redisCommands = invalidBlocks.map(function (block) {
				return ['zrem', config.coin + ':blocks:candidates', block.serialized];
			});

			redisClient.multi(redisCommands).exec(function (error, replies) {
				if (error) {
					log('error', logSystem, 'Error removing invalid blocks: %j', [error]);
					innerCallback(true);
					return;
				}
				log('info', logSystem, 'Successfully removed %d invalid blocks from database', [invalidBlocks.length]);
				innerCallback(null, invalidBlocks.length);
			});
		}
	], function (error) {
	...
	async.waterfall([
			...
			redisClient.zrange(config.coin + ':blocks:candidates', 0, -1, 'WITHSCORES', function (error, results) {
				...
				for (let i = 0; i < results.length; i += 2) {
					...
					blocks.push({
						...
						minerScore: parts.length >= 8 ? parts[7] : parts[5] // Individual miner's score for PPLNS
					});
				}
		...
		function (blocks, callback) {
			async.filter(blocks, function (block, mapCback) {
				...
				let rpcMethod = config.blockUnlocker.useFirstVout ? 'f_block_json' : 'getblockheaderbyheight';
				let postRequest = config.blockUnlocker.useFirstVout ? { hash: block.hash } : { height: blockHeight };
				
				apiInterfaces.rpcDaemon(rpcMethod, postRequest, function (error, result) {
					if (error) {
						log('error', logSystem, 'Error with %s RPC request for block %s - %j', [rpcMethod, block.serialized, error]);

						// If block not found or invalid height, remove it from redis
						let isInvalidBlockError = (
							error.message && (
								error.message.includes('Requested hash wasn\'t found in main blockchain') ||
								error.message.includes('height that is higher than the current blockchain height') ||
								error.code === -2
							)
						);

						if (isInvalidBlockError) {
							log('warn', logSystem, 'Removing invalid block %s from candidates (height %d)', [block.hash, block.height]);
							let letExe = [];
							letExe.push(['zrem', config.coin + ':blocks:candidates', block.serialized]);
							redisClient.multi(letExe).exec(function (redisError, replies) {
								if (redisError) {
									log('error', logSystem, 'Error with cleaning up invalid block in redis %j', [redisError]);
									return;
								}
								log('info', logSystem, 'Successfully removed invalid block %s from database', [block.hash]);
							});
						}
					...
					if(config.blockUnlocker.useFirstVout) {
						if (!result.block) {
							log('error', logSystem, 'Error with f_block_json RPC request for block %s - %j', [block.serialized, error]);
							block.unlocked = false;
							mapCback();
							return;
						}
						...
						blockHeader = result.block_header;
					}
					...
					if (config.blockUnlocker.useFirstVout) {
						apiInterfaces.rpcDaemon('f_transaction_json', {
							hash: blockHeader.transactions[0].hash
							...
							// If masternode and staking enabled phase, loop over everything except last 2 masternode and staking rewards
							if(blockHeight >= 259200) {
								block.reward = 0;
								for(let i = 0; i < vout.length; i++) {
									if(i == (v.out.length-1) || i == (v.out.length-2)) {} else {
										block.reward += v.out[i].amount;
									}
								}
							}
						});
		...
		function (blocks, callback) {
			...
			blocks.forEach(function (block) {
				...
				// Store participants list with their scores (address:score pairs, comma-separated)
				if (block.workerScores && Object.keys(block.workerScores).length > 0) {
					// Group scores by address (aggregate workers for same address)
					let addressScores = {};
					Object.keys(block.workerScores).forEach(worker => {
						let address = worker.split('.')[0]; // Extract address part
						if (!addressScores[address]) {
							addressScores[address] = 0;
						}
						addressScores[address] += parseFloat(block.workerScores[worker]) || 0;
					});

					// Convert to "address:score,address:score" format
					block.participants = Object.keys(addressScores)
						.map(addr => `${addr}:${addressScores[addr]}`)
						.join(',');
				} else {
					block.participants = `${block.login}:${block.minerScore}`; // Only the finder with their score
				}

				orphanCommands.push(['hset', config.coin + ':blocks:participants', block.height, block.participants]);
...
// 				})
		...
		function (blocks, callback) {
			...
			blocks.forEach(function (block) {
				...
				unlockedBlocksCommands.push(['hset', config.coin + ':blocks:participants', block.height, block.participants]);
...
// Clean up invalid blocks on startup
log('info', logSystem, 'Checking for invalid blocks on startup...');
cleanupInvalidBlocks(function () {
	log('info', logSystem, 'Startup cleanup complete, starting unlocker...');
	setTimeout(runInterval, config.blockUnlocker.interval * 1000);
});
