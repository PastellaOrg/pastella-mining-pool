			...
			async.filter(blocks, function (block, mapCback) {
				...
				apiInterfaces.rpcDaemon(rpcMethod, postRequest, function (error, result) {
					if (error) {
						...
						console.log(error);
						// If block not found in database then remove it from redis
						if(error == "Internal server error: Requested hash wasn't found in main blockchain") {
							console.log('do');
							let letExe = [];
							letExe.push(['zrem', config.coin + ':blocks:candidates', block.serialized]);
							redisClient.multi(letExe).exec(function (error, replies) {
								if (error) {
									log('error', logSystem, 'Error with cleaning up data in redis for orphan block(s) %j', [error]);
									return;
								} else {
									log('info', logSystem, 'Defective block removed!');
								}
							})
						}
