...
function getConfigDecimals() {
  try {
    const configPath = path.join(process.cwd(), 'config', 'pool.json');
    const config = JSON.parse(fs.readFileSync(configPath, 'utf8'));
    return config.pool.decimals || 8;
  } catch (error) {
    console.warn('Warning: Could not read pool.json, using default 8 decimals');
    return 8;
  }
}
